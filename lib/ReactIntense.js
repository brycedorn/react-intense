...
export default class ReactIntense extends React.Component {
  ...
  positionTarget () {
    ...
    const newX = mouseCurr.x + (mouseDest.x - mouseCurr.x) * 0.05
    const newY = mouseCurr.y + (mouseDest.y - mouseCurr.y) * 0.05
  ...
  _renderViewer () {
    ...
    return this.state.visible ? (
      <div ref='container'>
        <figure className='ri-container' style={{opacity: this.state.visible ? 1 : 0}}>
          <img src={this.props.src} style={transformStyle} onLoad={this._onLoad.bind(this)}/>
    ...
    return (
      <div className='ri-wrapper'>
        <a 
          ...
          ref='thumbnail' 
          style={{backgroundImage:'url('+this.props.thumbnailSrc+')'}} 
        />
        {this._renderViewer()}
      </div>
    )
